...
int coproc_init(size_t shm_max_size)
{
	...
	if ((tmp_env = (char **)malloc((i + 1) * sizeof(char *))) != NULL) {
		...
		for (i = 0; environ[i] != NULL; i++) {
			if ((tmp_env[i] = malloc(strlen(environ[i]) + 1)) !=
			    NULL)
...
#undef COND_SET_FLAG
	...
	if (flags & COPROC_DETACHED) {
		...
		detached_pid =
		    syscall(SYS_clone, clone_flags, NULL, NULL, NULL);
...
#define COND_SET_ATTR(flg, res, val)                                           \
	if ((flags & flg) && (!attrib || set_limit(res, attrib->val)))         \
	return -1

		COND_SET_ATTR(COPROC_SET_AS_SIZE, RLIMIT_AS,
			      address_space_size);
		COND_SET_ATTR(COPROC_SET_CORE_SIZE, RLIMIT_CORE,
			      core_file_size);
...
#undef COND_SET_ATTR
		...
		if (flags & COPROC_SET_SCHED) {
			...
			if ((!attrib) ||
			    sched_setscheduler(0, attrib->scheduling_policy,
					       &param)) {
				return -1;
			}
		}
